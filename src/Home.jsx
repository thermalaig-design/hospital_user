...
const Home = ({ onNavigate, onLogout, isMember }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (isMenuOpen) {
        ...
        // Only close if clicking on overlay, not sidebar
        if (isOverlayClick) {
          setIsMenuOpen(false);
        }
  ...
  // Disable page scroll and interactions when notifications dropdown is open
  useEffect(() => {
    if (isNotificationsOpen) {
  ...
  return (
    <div
      ...
      className={`bg-white min-h-screen flex flex-col relative ${isMenuOpen ? 'overflow-hidden max-h-screen' : ''}`}
    >
      {/* Navbar */}
      <div className="bg-white border-gray-200 shadow-sm border-b px-6 py-6 flex items-center justify-between sticky top-0 z-50 mt-6 transition-all duration-300 pointer-events-auto">
        <button
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              className="notification-button p-2 rounded-xl transition-colors relative hover:bg-gray-100"
            >
            ...
            {isNotificationsOpen && (
              <>
                {/* Transparent overlay covers the page and blocks interaction */}
                <div
