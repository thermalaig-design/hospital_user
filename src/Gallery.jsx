...
import { fetchAllGalleryImages, fetchGalleryFolders } from './services/galleryService';
...
export function Gallery({ onNavigate }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const allFolders = await fetchGalleryFolders();
  ...
  const filteredImages = useMemo(() => {
    ...
    if (!selectedFolder || !currentFolderName) return [];
    return images.filter(img => img.folderName === currentFolderName);
  }, [images, selectedFolder, currentFolderName]);

  // Helper to get a cover image for a folder (first image found)
  const getFolderCover = (folder) => {
    return images.find(img => img.folderName === folder.name)?.url || null;
  };
          ...
          {!isLoading && folders.length > 0 && !selectedFolder && (
            <div className="grid grid-cols-2 sm:grid-cols-2 gap-4 mb-8 mt-4">
              {folders.map(folder => {
                ...
                const count = images.filter(img => img.folderName === folder.name).length;
                return (
                  <div
                    key={folder.id}
