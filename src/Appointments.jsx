...
const Appointments = ({ onNavigate, appointmentForm, setAppointmentForm }) => {
  ...
  const handleSearchChange = async (value) => {
    ...
    // Always try searching in DB by name OR department
    try {
      const { data } = await supabase
        ...
        .or(`consultant_name.ilike.%${term}%,department.ilike.%${term}%`);
  ...
  // Clear a family member selection and reset someone-else fields
  ...
  const handleSpecialtySelect = async (specialty) => {
    ...
    try {
      ...
      let query = supabase
        ...
        .eq('is_available', true);
      if (deptKeyword) {
        // Support pipe-separated keywords (OR matching) e.g. for Dental & Oral
        if (deptKeyword.includes('|')) {
          const keywords = deptKeyword.split('|').map(k => k.trim());
          const orClause = keywords.map(k => `department.ilike.%${k}%`).join(',');
          query = query.or(orClause);
        } else {
          query = query.ilike('department', `%${deptKeyword}%`);
        }
      }
      const { data, error: fetchErr } = await query;
      if (fetchErr) {
        console.error('Error fetching specialty doctors:', fetchErr);
  ...
  const handleSubmit = (e) => {
    ...
    const appointmentData = {
      ...
      consultation_fee: selectedDoc?.consultation_fee || null,
          ...
          <div className="bg-gray-50 min-h-screen pb-6">
            ...
            <div className="bg-white mx-4 mt-4 rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="flex divide-x divide-gray-100 border-t border-gray-100">
                ...
                {doc.consultation_fee && (
                  <div className="flex-1 py-3 flex flex-col items-center gap-0.5">
                    <span className="text-green-600 font-bold text-base">₹{doc.consultation_fee}</span>
                    <span className="text-gray-400 text-[10px] text-center">Consult Fee</span>
                  </div>
                )}
                {doc.hospital_name && (
          ...
          <div className="bg-gray-50 min-h-screen pb-10">
            ...
            <div className="bg-white mx-4 mt-4 rounded-2xl shadow-sm border border-gray-100 p-4 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {doc.consultation_fee && (
                  <p className="text-green-600 text-xs font-semibold mt-0.5">₹{doc.consultation_fee} fee</p>
                )}
              </div>
            </div>
                ...
                <div className="mx-4 mt-4 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                  <div className="px-4 py-3 bg-green-50 border-b border-green-100">
                    <div className="flex items-center gap-2">
                      ...
                      {doc.consultation_fee && (
                        <span className="ml-auto text-xs font-semibold text-green-600">₹{doc.consultation_fee} fee</span>
                      )}
                    </div>
                    {(doc.general_opd_start || doc.general_opd_end) && (
                ...
                <div className="mx-4 mt-4 mb-6 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                  <div className="px-4 py-3 bg-indigo-50 border-b border-indigo-100">
                    <div className="flex items-center gap-2">
                      ...
                      {doc.consultation_fee && (
                        <span className="ml-auto text-xs font-semibold text-indigo-600">₹{doc.consultation_fee} fee</span>
                      )}
                    </div>
                    {(doc.private_opd_start || doc.private_opd_end) && (
            ...
            <div className="pb-32">
              ...
              <div className="mx-4 mt-3 bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {pendingAppointmentData.booking_for === 'someone' && pendingAppointmentData.patient_relationship && (
                    <div className="flex justify-between">
            ...
            <div>
              ...
              <div className="mb-6 px-6">
                ...
                <div className="flex bg-gray-100 p-1 rounded-2xl w-fit mx-auto">
                  ...
                  <button
                    type="button"
                    className={`py-2 md:py-3 px-4 md:px-6 rounded-xl text-sm md:text-base font-semibold transition-all duration-300 ${appointmentForm.bookingFor === 'someone'
                      ? 'bg-white text-indigo-600 shadow-md transform scale-105'
                      : 'text-gray-600 hover:text-indigo-500 hover:bg-gray-200'
                      }`}
                    onClick={() => {
              ...
              <div className="px-4 py-2">
                <form onSubmit={handleSubmit} className="space-y-6 max-w-md mx-auto px-2">
                  ...
                  {/* ── Family Member Picker (For Someone Else only) ── */}
